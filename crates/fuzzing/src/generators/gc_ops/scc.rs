...
impl StronglyConnectedComponents {
    ...
    ///
    /// `nodes` is the node set.
    /// `successors(n)` yields edges `n -> succ`.
        ...
        S: Iterator<Item = RecGroupId>,
    {
        ...
        // DFS index and lowlink for each node.
        //
            ...
            match event {
                ...
                DfsEvent::Post(node) => {
                    ...
                    if node_idx == node_low {
                        ...
                        let start = component_nodes.len() as u32;
    ...
    /// Iterate SCCs (in the same reverse-topological order as the inliner:
    /// successors come before predecessors in the SCC DAG).
        ...
        S: Iterator<Item = RecGroupId>,
    {
        loop {
            ...
            if let DfsEvent::Pre(node) = event {
                ...
                // AfterEdge + (maybe Pre) per successor, plus Post.
