...
/// How many instructions a type may cost to build inline before it gets a
/// shared prototype instead. Small enough to keep deep graphs bounded, large
/// enough that ordinary shallow types are still built fresh at each use.
const MAX_INLINE_CONSTRUCTION: u32 = 8;
...
impl GcOps {
    ...
    pub fn to_wasm_binary(&mut self) -> Vec<u8> {
        ...
        let proto_order = self.types.prototype_types(MAX_INLINE_CONSTRUCTION);
        ...
        // Refill the prototypes at the top of every iteration, so the ops
        // below never read a null local and each iteration allocates a fresh
        // set of objects for the collector to find and reclaim. `proto_order`
        // is in rank order, so a prototype's referents are already built.
        for tid in &proto_order {
            crate::generators::gc_ops::types::emit_new(
                *tid,
                &mut func,
                &self.types,
                &protos,
            ...
            Self::ArrayNewDefault { type_index: x } => {
                ...
                match array_element(types, encoding_order, x) {
                    ...
                    Some(element) if !element.field_type.is_defaultable() => {
                        element.field_type.emit_default_const(
                            func,
                            types,
                            protos,
                            type_ids_to_index,
                    ...
                    Some(fields) if !fields.is_empty() => {
                        ...
                        match mutable_field {
                            Some(idx) => {
                                ...
                                func.instruction(&Instruction::Else);
                                func.instruction(&Instruction::LocalGet(typed_local));
                                fields[idx].field_type.emit_default_const(
                                    func,
                                    types,
                                    protos,
                                    type_ids_to_index,
