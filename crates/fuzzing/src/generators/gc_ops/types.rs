...
impl Types {
    ...
    pub fn break_type_cycles_in_rec_groups(&mut self) {
        ...
        for (_g, ids) in members.iter() {
            ...
            // Simple cycle detection: DFS from each node, if we revisit a node in the
            // current path, we found a cycle. Break it by clearing supertype on the
            // node that would create the cycle.
            ...
            for &start in ids {
                ...
                loop {
                    if path_set.contains(&cur) {
                        // Found a cycle so clear supertype on cur to break it.
    ...
    pub fn merge_rec_groups_via_scc(&mut self, rec_groups: &BTreeMap<RecGroupId, Vec<TypeId>>) {
        ...
        for groups in sccs.iter() {
            ...
            // Deterministic canonical "keep" group: smallest RecGroupId in the SCC.
            let keep = *groups.iter().min().unwrap();

            // Merge every other group into `keep` by rewriting only the members of that group.
            for &g in groups {
                if g == keep {
                    continue;
                }
    ...
    /// Assumes no cycles (per current project assumption). If a cycle exists,
    /// the returned order will be incomplete and the debug_assert will fire.
    ...
    /// Sorts the types by their supertype (supertype before subtype).
